...
setTimeout(() => {
  ...
  const listingDataGenerator = () => {
    for (let i = 0; i < 100; i += 1) {
      ...
      const thingsToDoInt = getRandomInt(mockData.activities.length);
      const locationLatLon = getRandomInt(mockData.locations.length);
      const hostGuidebooksInt = getRandomInt(mockData.guideBks.length);

      const theQuery = `INSERT INTO listings (features, things_to_do, lat_location, lon_location, host_guidebook) VALUES ('${mockData.features[featuresInt]}', '${mockData.activities[thingsToDoInt]}', ${mockData.locations[locationLatLon][0]}, ${mockData.locations[locationLatLon][1]}, '${mockData.guideBks[hostGuidebooksInt]}')`;
